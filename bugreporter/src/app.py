...
def config(bot, update):
    ...
    button_list = []
    for board in boards:
        button_list.append(InlineKeyboardButton(board.name, callback_data=f'bid{board.id}'))

    reply_markup = InlineKeyboardMarkup(utils.build_menu(button_list, n_cols=2))
    bot.send_message(chat_id=update.message.chat_id, text="Please, choose a board", reply_markup=reply_markup)
...
def button(bot, update):
    ...
    if query.data.startswith('bid'):
        ...
        button_list = []
        for lst in lists:
            button_list.append(InlineKeyboardButton(lst.name, callback_data=f'lid{lst.id}'))
        ...
        bot.send_message(chat_id=query.message.chat_id, text=f'Congrats! You are ready to report bugs!')
    else:
        bot.edit_message_text(chat_id=query.message.chat_id, message_id=query.message.message_id,
                              text=f'Something went wrong. Please reconfigure bot with the /config command')
...
def file_handler(bot, update):
    ...
    if update.message.document or update.message.photo:
        if update.message.caption and '#bugs' in update.message.caption:
            attach = update.message.document if update.message.document else update.message.photo[-1]
            ...
            if trello_list:
                ...
                card.attach(file=file.download_as_bytearray(), mimeType=mime_type, name=file_name)
                if card:
...
def main():
    ...
    :return:
    """
    updater = Updater(token=settings.TELEGRAM_TOKEN, request_kwargs=REQUEST_KWARGS)
    dispatcher = updater.dispatcher

    dispatcher.add_handler(CommandHandler('start', start))
    dispatcher.add_handler(MessageHandler(Filters.photo | Filters.document, file_handler))
